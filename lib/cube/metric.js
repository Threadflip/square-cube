  ...
  function getter(request, callback) {
    ...
    if (stream) {
      ...
      if (streams) {
        ...
        ? function(time, value) { if (value) callback({time: time, value: value, id: id}); }
        : function(time, value) { if (value) callback({time: time, value: value}); });
      }
      else
      {
        streams = streamsByTier[tier.key] = {
        ...
        (function poll(streams){
          ...
          util.log(' >>> + about to filter active connections : ' + streams.active.length);
          streams.active = streams.active.concat(streams.waiting).filter(open);
          util.log(' >>> - after filtering active connections : ' + streams.active.length);
          streams.waiting = [];
          ...
          else
          {
            streams.active.forEach(function(stream) {
              util.log('stream : ', stream);
              measure(stream.expression, streams.time - streams.tier.key, streams.time, streams.tier, function(time, value) {

                if (stream.callback.closed) {
                  // callback closed already
                  util.log(' >>> callback closed');
                  return;
                }
                ...
                if (value !== undefined) {
                  ...
                  util.log(' >>> sending : ');
                  util.log(' >>>     ' + metric.time);
                  util.log(' >>>     ' + metric.value);
                  if (stream.id) util.log(' >>>     ' + metric.id);
                }
                else {
                  util.log(' >>> no value : ' + value);
                }
              });
            });
          }

          // TODO tier still are obscure to me.
          streams.time = streams.time + streams.tier.key;
          // calculate next step
          var timer = streams.tier.key;// - tier.ceil(new Date(Date.now()));
